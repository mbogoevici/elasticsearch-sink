...
public class TestTransportModeWithId {
    ...
    public void testSinkPassthrough() throws Exception {
        outputChannel.send(MessageBuilder.withPayload(MockUtils.SOURCE_INPUT).build());
        Mockito.verify(client, Mockito.times(1)).index((IndexRequest) Mockito.argThat(new ReflectionEquals(MockUtils.getIndexRequestWithId())));
    }
}
